...
function loadChat(forceBottom = false){

    if(receiver == 0) return;

    let box = document.getElementById("chatBox");

    let oldScrollHeight = box.scrollHeight;
    let oldScrollTop = box.scrollTop;

    let shouldScroll =
    oldScrollHeight - oldScrollTop - box.clientHeight < 120;
    ...
    .then(data => {
        ...
        if(box.innerHTML.trim() === data.trim()){
            return;
        }

        box.innerHTML = data;

        if(forceBottom || shouldScroll){
            box.scrollTop = box.scrollHeight;
        }else{
            box.scrollTop =
            box.scrollHeight - oldScrollHeight + oldScrollTop;
        }
    });
}
